...
import { vertexShader, fragmentShader } from './Grass_shaders'
...
export class GrassGeometry extends THREE.BufferGeometry {
  ...
  computeBlade(center : Array<number>, index = 0) {
...
const cloudTexture = new THREE.TextureLoader().load('models/Grass/cloud.jpg')
cloudTexture.wrapS = cloudTexture.wrapT = THREE.RepeatWrapping

export default class Grass extends THREE.Mesh {
  geometry : GrassGeometry;
  material : THREE.ShaderMaterial;
  constructor(size : number, count : number) {
    const grass_geometry = new GrassGeometry(size, count)
    ...
    super(grass_geometry, grass_material)
    this.geometry = grass_geometry;
    this.material = grass_material;
  ...
  update() {
      ...
      // console.log()
      if (this.material)
        this.material.uniforms.uTime.value = time / 3;
